    ...
    <div>
        <div class="container">
            <div class="handle-box">
                <el-button v-action:storage:create :icon="Plus"
                           type="primary"
                           @click="addVisible = true;Object.assign(form, new StorageImpl());">
                    新增对象存储
                </el-button>
            </div>
            <el-table ref="multipleTable" :data="tableData" border class="table" header-cell-class-name="table-header">
                ...
                <el-table-column align="center" label="标识" prop="storageId"></el-table-column>
                <el-table-column label="类型" prop="type" width="120">
                    <template #default="{ row }">
                        <el-tag class="ml-2" type="success">{{ row.type }}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column label="Bucket名称" prop="bucketName"></el-table-column>
                <el-table-column label="Endpoint" prop="endpoint"></el-table-column>
                <el-table-column label="访问地址" prop="address"></el-table-column>
                <el-table-column label="存储目录" prop="storagePath"></el-table-column>
                <el-table-column label="默认对象存储" prop="isDefault">
                    <template #default="{ row }">
                        <el-tag v-if="row.isDefault" v-action:storage:markAsDefault effect="dark" link>默认使用</el-tag>
                        <el-popconfirm v-else title="设为默认?" @confirm="handleMarkAsDefaultConfig(row)">
                            <template #reference>
                ...
                <el-table-column label="操作">
                    <template #default="scope">
                        <el-button-group>
                            <el-button v-action:storage:update :icon="Edit" text @click="handleEdit(scope.row)">
                                编辑
                            </el-button>
                            <el-button v-action:storage:delete :icon="Delete" class="red" text
        ...
        <el-dialog v-model="addVisible" title="新增" width="35%">
            <el-form label-width="100px">
                ...
                <el-alert :closable="false" center show-icon
                          style="margin-bottom: 18px" title="以下所有输入都支持环境变量，使用语法为 {{VARIABLE_NAME}}"/>
                <el-form-item v-if="form.type!=='LOCAL'" label="Endpoint">
                    <el-input v-model="form.endpoint" placeholder="对象存储资源的Endpoint"/>
                </el-form-item>
                <el-form-item v-if="form.type!=='LOCAL'" label="Bucket 名称">
                    <el-input v-model="form.bucketName" placeholder="对象存储资源的桶名称"/>
                </el-form-item>
                <el-form-item v-if="form.type!=='LOCAL'" label="AccessKey">
                    <el-input v-model="form.accessKey"/>
                </el-form-item>
                <el-form-item v-if="form.type!=='LOCAL'" label="SecretKey">
        ...
        <!-- 修改弹出框 -->
        <el-dialog v-model="editVisible" title="新增" width="35%">
            <el-form label-width="100px">
                <el-form-item label="名称">
                    <el-input v-model="form.name"/>
                </el-form-item>
